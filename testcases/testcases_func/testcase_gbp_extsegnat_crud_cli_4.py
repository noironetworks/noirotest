...
def main():
    ...
    test = testcase_gbp_extsegnat_crud_cli_4()
    test.test_runner()
    sys.exit(1)

class  testcase_gbp_extsegnat_crud_cli_4(object):
    """
    This is a GBP NAT CRUD TestCase
    """
    # Initialize logging
    _log = logging.getLogger()
    hdlr = logging.FileHandler('/tmp/testcase_gbp_extsegnat_crud_cli_4.log')
    ...
    def test_runner(self):
        ...
        """
        test_name = 'TESTCASE_GBP_EXTERNAL_SEGMENT_CRUD_4'
        self.write_neutron_conf()
        self._log.info("\nSteps of the TESTCASE_GBP_EXTERNAL_SEGMENT_CRUD_4 to be executed\n")
        testcase_steps = [self.test_step_CreateExternalSeg,
                          self.test_step_CreateNaTPoolWoExtSeg,
    ...
    def test_step_CreateNaTPoolWoExtSeg(self):
        """
        Create a NAT Pool without any reference to the existing Ext Seg
        """
        self._log.info("\nStep: Create the NAT Pool Without any reference to the existing External Segment\n")
        self.natpool_id = self.config.gbp_policy_cfg_all(1,'natpool',self.nat_pool_name,ip_pool=self.natippool)
