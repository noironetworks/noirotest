...
class testcase_gbp_extsegnat_crud_cli_2(object):
    ...
    def test_step_CreateExternalSegWithDestRouteNexthop(self):
        ...
        self.specific_gateway = re.sub("./\d+$", "100", self.cidr)
        extseg = self.config.gbp_policy_cfg_all(
            1, 'extseg', self.extseg_name, external_route='destination=2.3.4.0/24,nexthop=')
        if extseg == 0:
