...
def attach_fip_to_vms(tnt,mode):
        ...
        for vm in vms:
            ...
            fip_id = fip_id.decode('utf-8').split()[0]
            cmd1 = ('source ~/%s && neutron --os-project-name %s' %(RCFILE,tnt)+
            ...
            fixed_port_id = fixed_port_id.decode('utf-8').split()[0]
...
def dump_results(mode):
    text = subprocess.getoutput("grep -r %s-SANITY /tmp/test_sanity.log" %(mode)) 
...
class crudGBP(object):
    ...
    def create_policy_target(self,tnt):
        ...
        if tnt == tnt2:
            ...
            for vm,pt in {self.vms[0]:'pt5',self.vms[1]:'pt6'}.items():
        ...
        print(gbp_vm_ntk_ip)
        if 0 in list(gbp_vm_ntk_ip.values()):
    ...
    def install_tenant_vms(self,tnt):
        ...
        %(tnt))
        az = neutron.alternate_az(AVZONE)
        for vm,prop in gbp_vm_ntk_ip[tnt].items():
...
class sendTraffic(object):
    ...
    def generate_vm_prop(self,tnt,ext=False):
        ...
        properties = {}
        for vm,prop in ml2_vm_ntk_ip[tnt].items():
           pingable_ips = []
           for val in list(ml2_vm_ntk_ip[tnt].values()):
    ...
    def get_epg_vms(self,tnt,tag):
        ...
        if tag == 'intra_epg':
            for vm,prop in gbp_vm_ntk_ip[tnt].items():
                ...
                pingable_ips = []
                for val in list(gbp_vm_ntk_ip[tnt].values()):
                    if val['tag'] == tag:
    ...
    def traff_from_gbp_tenant(self,tnt,traffic_type,ext=False,
        ...
        failed_traff = 0
        for vm,vm_property in test_vms.items():
            if ext:
