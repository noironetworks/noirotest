...
class DNAT_ExtGw_to_VMs(object):
    ...
    def test_runner(self,vpc=0):
        ...
        test_list = [
                    #self.test_1_traff_with_no_prs, #JISHNU
        ...
        for test in test_list:
            try:
               if test()!=1:
                  ...
                  self._log.info("%s_%s == FAILED" %(self.__class__.__name__.upper(),string.upper(test.__name__.lstrip('self.'))))
               else:
                     self._log.info("%s_%s == PASSED" %(self.__class__.__name__.upper(),string.upper(test.__name__.lstrip('self.'))))
